...
BALANCED_TEMPERATURE = 800
BALANCED_EMITION = 500
...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    if (float(temperature) < float(BALANCED_TEMPERATURE) and float(neutrons_emitted) > float(BALANCED_EMITION)) and (float(temperature) * float(neutrons_emitted) < float(CRITICALITY_PRODUCT)):
        return True
